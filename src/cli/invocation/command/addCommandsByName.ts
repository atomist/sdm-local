...
import { YargBuilder, promptForAChoiceWhenNecessary } from "./support/yargBuilder";
        ...
        builder: args => {
            ai.client.commands.forEach(hi => {
                args.withSubcommand({
                    ...
                    conflictResolution: promptForAChoiceWhenNecessary(`run ${hi.name} (running command by name)`, ai.client.name),
                });
            });
            return args;
        },
